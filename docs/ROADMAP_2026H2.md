...
### Дорожка A — агент может сделать сейчас (4)

| Пункт | Замер 26-08-2026 |
|---|---|
| B6 «Вынести сохраненные веб-страницы из git» (L63 в редакции до этого прохода) | 31 отслеживаемый файл под `*_files/` ≈ 1,8 МБ — объём мал, работа механическая |
| C2 «Привязать эпитеты к построфным CTS-URN» | `grep -c 'urn:cts' data/ramayana_epithets.json` → **0**, тогда как `data/*_markup_50.json` и `commentary_schema.json` URN уже несут — инфраструктура готова, применена не везде |
| C2 «Свести IAST-леммы с `sources/leonov_notes.json`» | файл существует (15 657 байт) — источник на месте |
| B3 «Полная RNG-валидация против tei_all» | `xmllint` на машине есть; пункт помечен «нужен xmllint» с 2026-06-12 и с тех пор не переспрашивался |
