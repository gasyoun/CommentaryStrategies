...
1. ✅ **DONE (H784)** — [`scripts/compare_editions_mbh.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/compare_editions_mbh.py): a standalone, pluggable comparator (`critical_loader` = BORI ISO-15919 `MBh{NN}.txt`, `vulgate_loader` = Nīlakaṇṭha jsonl filtered by `parva_no`) that emits BOTH witnesses' verse ids in the same `work.adhyāya.śloka` 3-part shape `compare_editions.py`/`build_edition_apparatus.py` already expect — no changes needed to the Rāmāyaṇa script itself, so its exact reference output (865/2106/66) still reproduces byte-for-byte.
2. ✅ **DONE (H784)** — `scripts/build_edition_apparatus.py` generalized via CLI args (`--input --outdir --title --work-label --other-key --chapter-label`), defaults reproduce the original Sundara run unchanged (verified: 865 clean-variant verses / 2106 loci / 66 sargas, exact match). `verse_key()` needed NO change — MBh ids are emitted in the same shape it already parses.
3. **Similarity gate + Cyrillic-contamination quarantine** already in `build_edition_apparatus.py` — reused as-is; MBh Vanaparva run found 0 Cyrillic contamination (that bug is Rāmāyaṇa-southern-corpus-specific).
...
| **BORI / Poona critical** | ✅ **OBTAINED 12-07-2026** — John Smith / Tokunaga e-text (ISO-15919 Roman, `MBh01–18.txt`, ~11 MB) from [bombay.indology.info](https://bombay.indology.info/mahabharata/statement.html), local at `mahabharata-nilakantha/bori-critical/` (**gitignored** — © BORI, "do not provide copies to others"). Provenance + format: [`mahabharata-nilakantha/BORI_CRITICAL_SOURCE.md`](https://github.com/gasyoun/CommentaryStrategies/blob/main/mahabharata-nilakantha/BORI_CRITICAL_SOURCE.md). **Data gate cleared; only the comparator engineering remains.** |
...
| **Comparator + variant apparatus** | ✅ **PILOT SHIPPED 12-07-2026 (Vanaparva, parva 3)** — [`scripts/compare_editions_mbh.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/compare_editions_mbh.py) + generalized `build_edition_apparatus.py` → [`data/edition_comparison_mbh/vanaparva/`](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/edition_comparison_mbh/vanaparva/README.md): 10 316 critical vs 11 859 vulgate verses (vulgate count matches the independent census exactly); 296 identical, 8520 variant pairs → 6442 clean (18 699 apparatus loci), 930 routed to the reworded layer; 3043 vulgate-only (2074 structural absence / 969 reworded), 744 footnote-candidate runs, 25 whole extra adhyayas. **Remaining 17 parvas: same two scripts, `PARVA_NO` arg — engineering done, only the per-parva run + review remains.** |

**MBh-specific work — DONE at pilot grade for Vanaparva; H784's original concern (Nīlakaṇṭha P/U/A/S ↔ BORI `parva.adhyāya.śloka` addressing, by-content not by-number alignment, structural-absence detection) is resolved by content-based book-level LCS + Jaccard fuzzy fallback — same method as Rāmāyaṇa, verified sane on spot-checked variant pairs.** **Related but distinct:** [csl-atlas `scripts/forensic/f8_mbh_census.py`](https://github.com/sanskrit-lexicon/csl-atlas) is the MBh *citation-locus* census (loci-fitting), not edition collation — same corpora, different task; consume its verse index, don't duplicate.
