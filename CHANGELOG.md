...
- **Gate mirror job — the required check now blocks until a session verdict (H3551)** (OxAlpha `zai-coding-plan/glm-5.3-flash`, 03-09-2026). Live dogfood on [PR #218](https://github.com/gasyoun/CommentaryStrategies/pull/218) proved the protection blocks merges while the gate is pending, but also exposed GitHub's binding of required checks to the Actions app: a bare commit status does not satisfy the policy. Fix: [scripts/oxalpha_gate.py](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/oxalpha_gate.py) gains `wait` (poll the authoritative `oxalpha-review` status; green only on a session pass verdict; fast-red on fail/neutral; timeout = blocked), and the workflow carries an Actions job named exactly `oxalpha-review` mirroring that status — it runs for every actor (skipped jobs count as success on GitHub, so dependabot PRs are deliberately not skipped) and can never fabricate a pass because the status it reads is writable only by `ensure`/`verdict`.

- **OxAlpha review gate REQUIRED on main — steps 3–4 flipped early by ruling (H3551)** (OxAlpha `zai-coding-plan/glm-5.3-flash`, 03-09-2026, MG «required now», wedge understood). New [scripts/oxalpha_gate.py](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/oxalpha_gate.py): `ensure` arms the required `oxalpha-review` status (in-scope → **pending**, docs/data-only → evidenced **success**), `verdict` is the session-only terminal writer (pass→success / fail→failure / neutral→**error**, `--evidence` mandatory, terminal immutable per SHA, new push re-arms), `sweep` re-arms all open PRs on a 30-min cron — covering dependabot PRs whose event token is read-only by GitHub design. [.github/workflows/oxalpha-gate.yml](https://github.com/gasyoun/CommentaryStrategies/blob/main/.github/workflows/oxalpha-gate.yml) reworked to arm+comment (request names in-scope files and the exact verdict command); shadow mode retired after a ~2-hour run. Branch protection (flipped after this PR merges): required context `oxalpha-review`, strict, `enforce_admins` — **`main` becomes PR-only** (direct pushes rejected: the required status never ran on them — this mechanically closes the H3551 process finding F9). §4 human approval adapted to the single-author constraint (self-approval impossible): gate binds admin merges, review requests flag security/production paths, the merge act is the recorded approval. Design doc rewritten to REQUIRED state (§3/§4/§6/§7).
...
- **review-api draft saves are compare-and-set (H3551)** (OxAlpha `zai-coding-plan/glm-5.3-flash`, 03-09-2026). `putDraft()` in [review-api/src/drafts.js](https://github.com/gasyoun/CommentaryStrategies/blob/main/review-api/src/drafts.js) enforced its optimistic-versioning contract with a read-check-write across two awaited D1 statements, so two concurrent PUTs carrying the same expected version both read `version=N`, both wrote `version=N+1`, and both answered 200 — the first save was silently lost while the reviewer's UI reported success. The version check now lives inside the single write statement D1 executes atomically (expected-0 upsert guarded by `WHERE drafts.version=0`; expected>0 `UPDATE … WHERE version=?`), and 0 changed rows returns 409 with the current remote draft. Regression tests in [review-api/test/worker.test.js](https://github.com/gasyoun/CommentaryStrategies/blob/main/review-api/test/worker.test.js) reproduce the overlap (failed `200,200` before the fix) and cover the re-save-after-409 UPDATE path. Found by the H3551 OxAlpha 30-day retrospective review, slice #167.
### Added

- **OxAlpha 30-day retrospective code review — evidence report + future-gate design (H3551)** (OxAlpha `zai-coding-plan/glm-5.3-flash`, 03-09-2026). [reports/OXALPHA_30D_CODE_REVIEW_2026-08-26.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/reports/OXALPHA_30D_CODE_REVIEW_2026-08-26.md): ten risk-ranked executable-code slices from the fixed 26-07..25-08-2026 window (#167 #170 #159 #183 #185 #178 #182 #175 #194 #181), each with independent Standards and Spec verdicts, exact base/head SHAs, exclusions, and a zero-unsupported-findings ledger — outcome: 0 P0, 1 P1 (review-api draft lost-update, fixed by [#214](https://github.com/gasyoun/CommentaryStrategies/pull/214)), 1 latent P2 (per-sarga submit vs 68-sarga server contract), 6 P3 hardening notes, 1 process finding (window work landed as direct `ai-wip` pushes to main). Canonical adapter bootstrap merged as [#213](https://github.com/gasyoun/CommentaryStrategies/pull/213). Wave 3 ships [docs/OXALPHA_STATUS_GATE_DESIGN_2026.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/OXALPHA_STATUS_GATE_DESIGN_2026.md) — executable-path matching, independent `oxalpha-review` status check (pass/evidence-backed-fail/neutral, never silent pass), extra human approval for security/production paths, rollout+rollback — **design only; no workflow, protection rule, or required check enabled**. Pointer stub at [docs/reviews/OXALPHA_RETROSPECTIVE_CODE_REVIEW_26-08-2026.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/reviews/OXALPHA_RETROSPECTIVE_CODE_REVIEW_26-08-2026.md).
...
- **Drain H3778 — C3 pilot: 20-stanza born-structured JSON annotations for Sundara 1, model II** (Sonnet 5 `claude-sonnet-5`, 02-09-2026). [docs/ROADMAP_2026H2.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/ROADMAP_2026H2.md) Workstream C, C3 — extends [C0.2](https://github.com/gasyoun/CommentaryStrategies/blob/main/ramayana-leonov/C0_SPECIMENS_SUNDARA1.md)'s 10-stanza specimen to 20 in the D2-decided model II (two-tier hybrid), assembled directly as JSON rather than retrofitted from prose: new [data/sundara1_pilot_c3_20.json](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/sundara1_pilot_c3_20.json), built by [`scripts/build_c3_sundara1_pilot.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/build_c3_sundara1_pilot.py), with the review packet at [ramayana-leonov/C3_PILOT_SUNDARA1_20.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/ramayana-leonov/C3_PILOT_SUNDARA1_20.md). Tier-1 print glosses reuse C0.2's already-calibrated wording verbatim for stanzas 1/3/6/7/8/10 and add four new ones (16/18/19/20) condensed from the same vetted tier-2 note (no new philological claim). Tier-2 digital notes are the existing Leonov/Kostina apparatus (`data/leonov_own_notes.json`, 14/20 stanzas already annotated). The C3 "parallel Govindarāja/Tilaka glosses" item reuses the existing prior-art segmenter [`scripts/extract_yellow_sargas.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/extract_yellow_sargas.py) (H268 WS-C2, pratīka + content-anchor verified, 0.887/0.931 precision) instead of a fresh hand-rolled parser — an earlier ad-hoc regex attempt in this same drain mis-segmented on real scan artifacts (duplicate/merged verse markers) and was discarded once the calibrated tool was found. Roadmap: two C3 checkboxes ticked; the third (Leonov/Kostina review) stays open with `{gate: human:leonov,kostina}`; the three data-gated Workstream-D/v1.0-carryover items (Gasparov-50, CLR contact, Averintsev/Losev) get explicit `{gate: data:…-not-acquired}` / `{gate: external:…}` annotations instead of silent open boxes; dated header bumped (was undated, tripping `roadmap_index_check`) and the `Uprava/ROADMAP_INDEX.md` Updated cell corrected from `—` to `02-09-2026` in a separate Uprava commit. Translation/interpretation of the Govindarāja/Tilaka layer into Russian is explicitly left to the C3 review step, not attempted mechanically.

## [1.26.2] - 2026-08-30
### Added

- **Drain A05 — B6 repo-side: tracked-binary census gate + rights triage for release assets** (OxAlpha `opencode/z-ai/glm-5.3-flash`, 29-08-2026, [PR #210](https://github.com/gasyoun/CommentaryStrategies/pull/210)). The «Большие `.docx/.pdf` — в релизы» checkbox's agent-executable half, done without inventing the rights act its own text routes to Дорожка C. The 29-08 census (magic-byte + extension detection, stdlib) confirms the 26-08 measurement: 8 tracked binaries, 20.9 MB, of which mechanically release-eligible today — **zero**; new stdlib tool [`scripts/release_binaries.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/release_binaries.py) classifies each (rights / timing / build-input in `VERDICTS`), snapshots the census to [data/binary_census.json](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/binary_census.json) (`--emit`, derive-don't-store), gates CI against silent regrowth (`--check`: a new tracked binary without a verdict reds the «Corpus integrity» job until it is classified and re-censused in one PR — the H2832/H3558 discipline becomes enforced rather than remembered), and provides a guarded `upload` path (dry-run by default, owner allowlist mandatory, the `rights` class never passes without `--rights-cleared`, i.e. a permission letter actually held by the owner). The full triage — both third-party scans (kazansky_1987.pdf 19.7 MB, 02_Lidova_31-66.pdf 0.4 MB) stay human decisions, the LP print master and three Tronsky article drafts await publication-timing/co-author consent, the two pandoc references are build inputs that stay in git — plus the owner's ~10-min-per-file upload прогон live in [docs/BINARY_RELEASES.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/BINARY_RELEASES.md). Verified: gate green on the clean tree and red (exit 1) on a probe tracked binary; upload dry-run plans correctly and hard-denies the rights class; `validate.py` 6049 files PASS. The publication decisions and the actual upload remain with the owner; the roadmap checkbox is ticked with that honest split note.

- **Radial + timeline panels of strategy evolution in `visualizations.html`** (OxAlpha `opencode/z-ai/glm-5.3-flash`, 29-08-2026, [PR #209](https://github.com/gasyoun/CommentaryStrategies/pull/209)). The §5.3 v1.0 carry-over «Radial / timeline визуализации эволюции стратегий» closes: panel 06 draws each corpus's publisher span on a 1940–2020 axis, panel 07 places the six corpora on chronologically ordered polar spokes with radius = IAST % (rings 25/50/75/100) and a dashed polyline joining the dated corpora in time order. Chronology is grounded only in what the corpus itself asserts — span ranges from [data/RIGHTS.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/RIGHTS.md) (Kalyanov 1950–1996, Vassilkov/Neveleva 1987–2005) and the handwritten essays' Ladomir subtitles (Grintser 2006, Erman 2009); Syrkin (no edition dates on file) and Leonov/Kostina (ongoing translation) render as dashed/hollow *undated* marks with a note, never with an invented year. Both panels live in the generator [`scripts/build_visualizations.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/build_visualizations.py) (constants `YEARS`; derive-don't-store, CI parity gate holds) and both react to the filter chips (`drawTimeline`/`drawRadial` in `sync()`; page note now says seven panels). Verified: rebuild byte-stable, `validate.py` 6044 files PASS, headless Playwright smoke on real Chart.js 4.4.1 — 6 span bars (2 flagged undated), 6 spokes + polyline + 2 «—» year labels, chip toggle cascades to both new panels, zero console errors.

- **Drain A05 — B6 repo-side: Zenodo release metadata parity + owner deposit runbook** (OxAlpha `opencode/z-ai/glm-5.3-flash`, 29-08-2026). The Zenodo checkbox's agent-executable half, done without inventing the human act of depositing: (1) `.zenodo.json` carried version `1.2.0` — the CFF version, not a release version — while `CITATION.cff` and the git tags lived at `v1.26.1`; fixed to `1.26.1` and its rights `notes` now also declare the Vālmīki layer (CC BY 4.0, Palaniappan grant per `data/valmiki_PERMISSION.md`), owed at deposit time. (2) New stdlib gate [`scripts/check_release_meta.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/check_release_meta.py) enforces `CITATION.cff` ↔ `.zenodo.json` version parity plus a structural minimum (title/license/version/creators-with-ORCID/date-released), wired into the CI corpus job; git-tag comparison degrades to a WARNING/SKIP where tags aren't fetched, so the gate never becomes a phantom in shallow checkouts and never false-fails the legal release-commit-before-tag interval. (3) New owner runbook [docs/ZENODO_DEPOSIT.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/ZENODO_DEPOSIT.md): one-time GitHub–Zenodo integration, concept-DOI-vs-version-DOI citing rule, SemVer-mapped versioning, the three-layer license split, post-deposit DOI wiring, and the explicit do-not-deposit list (`kazansky_1987.pdf` stays a rights act, Дорожка C). The deposit itself (zenodo.org account, ~15 min via the runbook) and DOI backfill into `CITATION.cff`/README remain with the owner; the roadmap checkbox is ticked with that honest split note.
- **Drain — interactive translator filters in `visualizations.html`** (OxAlpha `opencode/z-ai/glm-5.3-flash`, 28-08-2026, [PR #207](https://github.com/gasyoun/CommentaryStrategies/pull/207)). The last §5.3 v1.0 carry-over closed: the page now carries a filter strip — one chip per translator plus a master «все» chip — acting on all five panels at once. Bubble/radar/scatter toggle via Chart.js `setDatasetVisibility`; the Kazansky stacked bar is type-indexed (datasets A/B/V/G, not translators), so its `labels`+`data` are rebuilt from the same `D` on every toggle (derive-don't-store, no duplicated copy); the SVG heatmap redraws with active columns only. The filters live in the generator [`scripts/build_visualizations.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/build_visualizations.py), not in hand-edited HTML — CI's `git diff --exit-code` parity gate holds. Verified: generator output's `__DATA__` payload byte-identical to the previous build, `validate.py` 6044 files PASS, and a headless Playwright smoke on real Chart.js 4.4.1 (chip hides the translator on all four Chart.js panels and the heatmap; master clears and restores everything; kaz data round-trips exactly from `D`; zero console errors). The same pass truth-pass-ticked two stale C2 checkboxes in [docs/ROADMAP_2026H2.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/ROADMAP_2026H2.md) — «Привязать эпитеты к построфным CTS-URN» (half-done in PR #203, honest split note) and «Свести IAST-леммы с sources/leonov_notes.json» (done in PR #203; counts re-verified on disk before ticking: 45 lemmas → 15 entries, 24 attestations).

- **Drain B3 — TEI supplementary-data declaration in Article 3** (OxAlpha `opencode/z-ai/glm-5.3-flash`, 28-08-2026). [articles/article3_nilakantha.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/articles/article3_nilakantha.md) gains a «Дополнительные материалы» section declaring the machine-readable corpus as supplementary data: [tei/leonov.xml](https://github.com/gasyoun/CommentaryStrategies/blob/main/tei/leonov.xml) named explicitly, plus the five edition exports (kalyanov, vassilkov, erman, grintser, syrkin), all CTS-URN-addressed, four-axis `@ana`-marked and RelaxNG-valid (TEI P5 4.12.0, H3558), with the rights line pointing at [data/RIGHTS.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/RIGHTS.md). Physical attachment to the submitted manuscript stays at submission time per the roadmap's own priority note; for Article 4 (JAOS, unwritten) the block's carry-over is recorded in `.ai_state.md`. [docs/ROADMAP_2026H2.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/ROADMAP_2026H2.md) B3 checkbox ticked with the split note.

- **Drain A04 — B2: crosswalk from CTS-URNs to samskrtam.ru parallel-corpus IDs** (OxAlpha `opencode/z-ai/glm-5.3-flash`, 28-08-2026). The B2 checkbox's "data-gated — нужны их ID" premise dissolved on inspection: the corpus is the org's own [samskrtam.ru parallel corpus](https://samskrtam.ru/parallel-corpus/), and its anchor IDs are public in the page HTML. New [`scripts/samskrtam_map.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/samskrtam_map.py) (derive-don't-store, stdlib-only, `--emit`/`--check`) derives a mapping for every note URN → [`data/samskrtam_id_map.json`](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/samskrtam_id_map.json): **300 notes → 244 mapped · 6 anchor-missing · 50 not-in-corpus** (Syrkin's upanishads are outside the corpus). Anchor schemes verified against real HTML, not assumed: Ramayana kanda V carries verse-level `id="<sarga>.<verse>"` (its page is live though unlinked from `index_ramayana.html`), Ramayana I–III and all 18 Mahabharata parvas carry chapter-level `id="chapter_<n>"`; the six `anchor-missing` rows are honest findings, not mapper failures — suspect sample addresses surfaced by the corpus acting as control (Rām. I sarga 80 vs CE's 77; Rām. V 31.44 absent from the live page; Mbh. VI adhy. 115/120 beyond the page's `chapter_111`; Mbh. XVIII adhy. 6 vs CE's 5). Two operational findings recorded in the artifact's provenance: samskrtam.ru soft-404s (a missing page serves the corpus landing page with HTTP 200, exactly 33 857 bytes — HEAD checks prove nothing on this server), and the corpus pages' chapter splits diverge from critical-edition numbering. `samskrtam_map.py --check` wired into the CI corpus job; optional `samskrtam_ref` / `samskrtam_url` fields documented in [`data/commentary_schema.json`](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/commentary_schema.json); README corpus table + reproducibility section updated; [docs/ROADMAP_2026H2.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/ROADMAP_2026H2.md) B2 ticked with the owner-side residual (address reconciliation against the printed editions).

- **Drain — Contribution guidelines for indologists** (OxAlpha `opencode/x-preview-f-free`, 28-08-2026). [`CONTRIBUTING.md`](https://github.com/gasyoun/CommentaryStrategies/blob/main/CONTRIBUTING.md) rewritten from the 9-line universal stub into the indologist onboarding doc: path A — how to add a new translator (seven steps: `data/<t>_full.json` → 50-note gold markup → `translator` enum extension in `data/commentary_schema.json` → `scripts/profile_translator.py` → `scripts/build_pages.py` → the handwritten HTML essay); path B — how to mark up one note per the four axes (all enum values plus the canonical traps: realia belong to axis 2 `V`, never `B`; axis-4 canon is `P/K/D` with no `C`, per [docs/AXIS4_KD_DECISION.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/AXIS4_KD_DECISION.md); CTS URN derived, not invented, via `scripts/derive_urn.py`); what `scripts/validate.py` checks locally and in CI; the rights rules (no full translation texts, no protected scans). Closes the last §5.2 carry-over of v1.0 in [docs/ROADMAP_2026H2.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/ROADMAP_2026H2.md) («Перенесено живым»).

- **H3558 — Lane-A slice-5 residuals: real RelaxNG validation and the IAST crosswalk** (Opus 5 `claude-opus-5`, 26-08-2026). New [`scripts/validate_tei_rng.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/validate_tei_rng.py) runs `xmllint --relaxng` over `tei/*.xml` against a real tei_all (schema via `--schema` / `$TEI_ALL_RNG` / `schema/tei_all.rng`; SKIP + exit 0 when `xmllint` or the schema is absent, so it never turns into a phantom gate). The schema is deliberately **not** vendored — 1.1 MB of foreign RNG is the same ballast class as the web dumps below. The first real run against tei_all 4.12.0 found that every exported file was invalid: `listAnnotation` is reachable only through `model.standOffPart`, i.e. only inside `<standOff>`, never inside `<text>/<body>` — an error `ElementTree` cannot see in principle. New field `iast_crosswalk` on epithet entries of [`data/ramayana_epithets.json`](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/ramayana_epithets.json), emitted by the generator (`load_crosswalk` / `attach_crosswalk` in [`scripts/parse_formulas.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/parse_formulas.py)), never by hand: 45 IAST lemmas harvested from Kostina's book-V notes in [`sources/leonov_notes.json`](https://github.com/gasyoun/CommentaryStrategies/blob/main/sources/leonov_notes.json), all 45 resolving to a valid CTS-URN through the same `derive_urn.derive()`, matched to books 1–2 epithets by normalised Russian gloss → **15 entries, 24 attestations**; `grep -c 'urn:cts' data/ramayana_epithets.json` 0 → 24. The link is **lemmatic, not referential** — `attested_urn` says where the Sanskrit lemma is attested in book V, not where the Russian article stands in books 1–2 (`dhīmān` is attested at 5.1.3 of Hanumān, while «мудрый» in books 1–2 stands with Bharata, Viśvāmitra, Kuśa and Manu); field names were chosen so the address cannot be misread as the article's own. Documented in [ramayana-leonov/EPITHET_LAYER.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/ramayana-leonov/EPITHET_LAYER.md).

- **H3551 (OxAlpha) — CommentaryStrategies 30-day risk-ranked code review and future independent review gate: execution-ready five-layer plan staged** (OxAlpha (x-preview-f-free), plan authored 26-08-2026 by Codex Sol (gpt-5.6-sol)). [Plan index](https://github.com/gasyoun/CommentaryStrategies/blob/main/docs/PLAN_COMMENTARYSTRATEGIES_OXALPHA_CODE_REVIEW_HARDENING_2026Q3.md) fixes the 30-day window, ten-slice risk cap, independent Standards/Spec passes, evidence-only P0/P1 repair contract, canonical GitHub adapter, and an inactive future status-gate design.

- **H3346 — Q4.1 Type-D pilot concordance on the linkid library** (OxAlpha `opencode/x-preview-f-free`, 26-08-2026). First Type-D `commentary-citation` dataset built end-to-end through [sanskrit-util](https://github.com/sanskrit-lexicon/sanskrit-util) v0.10.0's linkid builders/validator (H3341): [`scripts/build_typed_link_sundara_concordance.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/build_typed_link_sundara_concordance.py) harvests √root citations from the 68-file lexical layer, accepts a tail only if it already exists in the [WhitneyRoots mw_roots inventory](https://github.com/gasyoun/WhitneyRoots/blob/main/crosswalk/mw_roots.json) (reuse-don't-mint; 89 off-inventory citations skipped and counted, never minted), emits 258 rows as `root:<SLP1>` ↔ `commentary:sundara-lexical:V.<s>.<v>` ([TSV](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/typed_link_sundara_concordance.tsv) + [JSONL](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/typed_link_sundara_concordance.jsonl), all records 0-error on `linkid_validate_link_record`), dedups against Leonov/Kostina's own 1058 notes with word-boundary-aware citation matching (156 unique / 102 verse-overlap / 0 root-overlap — [report](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/analysis/typed_link_sundara/dedup_vs_1058_report.md)), and renders the human gate via the shared csl-pyutil emitter (sheet_id `commentarystrategies-sundarakanda-typed-link-q41`, V9 manifest + screening + V13 identity gate; [sheet](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/analysis/typed_link_sundara/commentarystrategies-sundarakanda-typed-link-q41_review.html)). **No store write has happened or can happen before the vote**: `--apply-decisions` refuses unvoted/partial decisions files and writes only the confirmed-tier TSV/JSONL; apparatus stores untouched. Resolution note: [data/typed_link_sundara_concordance.README.md](https://github.com/gasyoun/CommentaryStrategies/blob/main/data/typed_link_sundara_concordance.README.md) (registered per-repo per TYPED_LINK_ID_GRAMMAR §5 D2b; kosha manifest deferred to Q2.1).

### Changed

- **H3558 — TEI notes moved to `<standOff>`; tracked web dumps unpinned from the index** (Opus 5 `claude-opus-5`, 26-08-2026). [`scripts/export_tei.py`](https://github.com/gasyoun/CommentaryStrategies/blob/main/scripts/export_tei.py) now emits `<TEI><teiHeader/><standOff><listAnnotation>` instead of `<text><body><listAnnotation>`; all six `tei/*.xml` regenerated (300 notes) and all six now validate green against TEI P5 4.12.0 tei_all, where before this pass none did. 31 tracked css/js/image files (≈ 1.8 MB) under `*_files/` were detached from the index (`git rm --cached`) and closed by a `*_files/` rule in `.gitignore`; the two parent saved `.html` pages **stay tracked** — `extract_leonov_notes.py` and `build_book_apparatus.py` read them, so a fresh clone renders them unstyled by design. History was **not** rewritten (`filter-repo` + force-push is a separate decision, not taken here), and the 20 MB `tronsky-XXX/sources/kazansky_1987.pdf` half of that roadmap item is re-classified as a rights act, not a mechanical move.
